...
class _ChangePageState extends State<JobsCollectionScreen> {
                        ...
                        child: ButtonTheme(
                          ...
                          child: Container(
                            ...
                            child: RaisedButton(
                              ...
                              textColor: Color(red),
                              color:
                                  enable ? Color(yellow) : Colors.grey.shade200,
                              onPressed: () {
                                if (enable) {
                                  showDialog(
                                      context: context,
  ...
  Future<void> redirect(checkTipButton) async {
    setState(() {
      _loading = true;
    });
    Map<String, String> headers = {
    ...
    var uri = checkTipButton
        ? Uri.parse(URL + "tipping-process/ready-to-tip")
            .replace(queryParameters: params)
        : Uri.parse(URL + "tipping-process").replace(queryParameters: params);

    var response = checkTipButton
        ? await http.post(
            uri,
            headers: headers,
          )

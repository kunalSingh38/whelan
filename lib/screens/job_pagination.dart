...
class _ChangePageState extends State<JobsScreen11> {
  ...
  Future _configData() async {
    ...
    setState(() {
      currency =
          jsonDecode(data)['data']['currency']['html_currency_code'].toString();
      prefs.setString('html_currency_code', currency);

      for (int i = 0;
          i < jsonDecode(data)['data']['payment_mode'].length;
          i++) {
        if (jsonDecode(data)['data']['payment_mode'][i]['payment_mode'] ==
            "Online-Square") {
          prefs.setString(
              'bearer_token',
              jsonDecode(data)['data']['payment_mode'][i]['bearer_token']
                  .toString());
          prefs.setString(
              'square_application_id',
              jsonDecode(data)['data']['payment_mode'][i]
                      ['square_application_id']
                  .toString());
          prefs.setString(
              'location_id',
              jsonDecode(data)['data']['payment_mode'][i]['location_id']
                  .toString());
        }
      }
    });
          ...
          builder: (context) => new AlertDialog(
            ...
            actions: <Widget>[
              ...
              new FlatButton(
                ...
                child: new Text("Yes", style: TextStyle(color: Color(red))),
              ),
            ],
          ),
        )) ??
